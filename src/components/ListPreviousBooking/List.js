...
import {
    FormControl, InputAdornment, Input, Accordion, AccordionDetails, Button, AccordionSummary, Box, Typography, Link, Badge,
} from "@mui/material";
...
const List = ({ setFetchChargerFromList, user, userBooking }) => {
    ...
    const pendingBookingsInfo = useMemo(() => {
        return userBooking.filter((booking) =>
            ...
            booking.status === STATUS_CHARGING
        );
    }, [userBooking]);
    ...
    const recentBookingsInfo = useMemo(() => {
        return userBooking.filter((booking) =>
            ...
            booking.status === STATUS_CHARGING_COMPLETED
        );
    }, [userBooking]);
    ...
    useEffect(() => {
        const helperFunction = async () => {
            const response1 = await Promise.all(pendingBookingsInfo.map(async (charger) => {
                ...
                return [fetchedChargerRequest.charger, charger.status, charger.timeSlot, charger.bookingId, charger.bookingDate, charger.price,];
            })
            );
            setPendingBookings(response1);
            const response2 = await Promise.all(
                recentBookingsInfo.map(async (charger) => {
                    const fetchedChargerRequest = await getUserAndChargers(charger?.uId, charger?.chargerId);
                    return [fetchedChargerRequest.charger, charger.status, charger.timeSlot, charger.bookingId, charger.bookingDate, charger.price,];
                })
            );
    ...
    const filteredBookings = useMemo(() => {
        ...
        if (searchParams.get("chargerType")) {
            ...
            filtered = filtered.filter(([charger]) => {
                if (
                    filterTypes.includes("level-1") &&
                    charger?.info.chargerType[0] === "Level 1"
                ) {
                    return true;
                } else if (
                    filterTypes.includes("level-2") &&
                    charger?.info.chargerType[0] === "Level 2"
                ) {
                    return true;
                } else if (
                    filterTypes.includes("level-3") &&
                    charger?.info.chargerType[0] === "Level 3"
                ) {
                    return true;
                } else {
                    return false;
        ...
        if (searchParams.get("sortBy") === "price") {
            filtered.sort((charger1, charger2) => {
                return (
                    Number(charger1[0]?.info?.price) - Number(charger2[0]?.info?.price)
                );
            });
        ...
        if (searchParams.get("from") || searchParams.get("to")) {
            ...
            if (fromDate || toDate) {
                filtered = filtered?.filter((charger) => {
                    ...
                    if (fromDate && toDate) {
                        if (
                            chargerDate.getTime() >= fromDate.getTime() &&
                            chargerDate.getTime() <= toDate.getTime()
                        ) {
                            return true;
                        }
                    } else if (fromDate) {
                        ...
                        <Box sx={{ display: "flex", gap: "1rem" }}>
                            ...
                            >
                                ...
                                {pendingBookingsInfo.length > 0 && (
                                    <Badge
                                        badgeContent={pendingBookingsInfo.length}
                                        color="success"
                                        sx={{ top: "-13px", left: "4px" }}
                            ...
                            <Link
                                style={{
                                    textDecoration: show === "recent" ? "underline" : "none",
                                    fontSize: "1rem",
                                    textUnderlineOffset: "8px",
                                    color: "antiquewhite",
                                    cursor: "pointer",
                            ...
                            >
                                ...
                                {recentBookingsInfo.length > 0 && (
                                    <Badge
                                        badgeContent={recentBookingsInfo.length}
                                        color="success"
                                        sx={{ top: "-13px", left: "4px" }}
                                        size="small"
                                        anchorOrigin={{ vertical: "top", horizontal: "right" }}
