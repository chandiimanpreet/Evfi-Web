...
const List = ({ setFetchChargerFromList, user, userBooking }) => {
  ...
  const pendingBookingsInfo = useMemo(() => {
    return userBooking.filter(
      (charger) =>
        charger.status === STATUS_REQUESTED ||
        charger.status === STATUS_ACCEPTED ||
        charger.status === STATUS_CHARGING
    );
  }, [userBooking]);
  ...
  const recentBookingsInfo = useMemo(() => {
    return userBooking.filter(
      (charger) =>
        charger.status === STATUS_DECLINED ||
        charger.status === STATUS_CANCELED ||
        charger.status === STATUS_CHARGING_COMPLETED
    );
  }, [userBooking]);
  ...
  useEffect(() => {
    const helperFunction = async () => {
      const response1 = await Promise.all(
        pendingBookingsInfo.map(async (charger) => {
          const fetchedChargerRequest = await getUserAndChargers(
            charger?.uId,
            charger?.chargerId
          );
          return [
